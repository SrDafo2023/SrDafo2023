...
apiRouter.patch("/users/:userId/role", isAdmin, async (req, res) => {
    try {
...
apiRouter.post("/subscriptions", isAuthenticated, async (req, res) => {
    try {
        ...
        await (0, subscription_service_1.createSubscription)(uid, planId);
...
apiRouter.post("/adoption-requests", async (req, res) => {
    try {
...
apiRouter.patch("/adoption-requests/:formId", async (req, res) => {
    try {
...
apiRouter.get('/appointments/occupied', async (req, res) => {
...
apiRouter.get("/appointments", async (req, res) => {
    try {
        ...
        const appointmentsRaw = snapshot.docs.map(doc => (Object.assign({ id: doc.id }, convertTimestampToDate(doc.data()))));
        ...
        const appointments = appointmentsRaw.map(appt => (Object.assign(Object.assign({}, appt), { clientName: clientsMap[appt.clientId] || '', petName: petsMap[appt.petId] || '', serviceName: servicesMap[appt.serviceId] || '' })));
...
apiRouter.get('/appointments/range', async (req, res) => {
    try {
...
apiRouter.get("/admin/top-products", isAdmin, async (req, res) => {
    try {
        const { startDate, endDate } = req.query;
...
apiRouter.get("/admin/top-services", isAdmin, async (req, res) => {
    try {
        const { startDate, endDate } = req.query;
